...
private:
	...
	vector<unsigned char> _vImageData;
...
public:
	Image();
	Image(unsigned int iWidth, unsigned int iHeight, vector<unsigned char>& vImageData, unsigned int iStreamId, const string& sTimestamp, const string& sResult);
	~Image();
	...
	const vector<unsigned char>& getImageData() const;
	void setImageData(const vector<unsigned char>& imageData);
